/* ================= NEO-TOKYO · shared components ================= */
const { useState, useEffect, useRef, useMemo, useCallback } = React;

/* ---- Lucide icon set (stroke paths from lucide.dev, ISC) ---- */
const ICONS = {
...
/* ---- Button ---- */
function Btn({ children, variant = '', icon, onClick, style, title }) {
  return (
    <button className={`btn ${variant ? 'btn-' + variant : ''}`} onClick={onClick} style={style} title={title}>
...
function Chip({ children, color, style }) {
  return (
    <span className="chip" style={{ ...(color ? { color, borderColor: color + '66' } : {}), ...style }}>
      {children}
    </span>
  );
}
...
/* ---- Eyebrow with kanji ---- */
function Eyebrow({ children, jp }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
      <span style={{ width: 22, height: 1, background: 'var(--accent)', boxShadow: '0 0 8px var(--accent)' }} />
      <span className="eyebrow">{children}</span>
      {jp && <span className="jp" style={{ fontSize: 11, color: 'var(--tx-faint)' }}>{jp}</span>}
    </div>
  );
}
...
/* ---- agent color lookup ---- */
const AGENTS = {
  claude:   { label: 'Claude Code', color: 'var(--claude)',   hex: '#62b4ff', glyph: '✳' },
  codex:    { label: 'Codex',       color: 'var(--codex)',    hex: '#86efac', glyph: '◇' },
  gemini:   { label: 'Gemini',      color: 'var(--gemini)',   hex: '#fbbf24', glyph: '✦' },
  opencode: { label: 'OpenCode',    color: 'var(--opencode)', hex: '#c084fc', glyph: '⬡' },
};
...
Object.assign(window, { Icon, Btn, Chip, Eyebrow, AGENTS, useCountUp, useCountUp });
